...
# Exploración, análisis y visualización 

¿Cuáles son los 5 productos, de los vendidos por unidades, con más ventas ? ¿Cuántas unidades de cada uno se han vendido ?


```{r a, warning=FALSE, echo =FALSE, fig.align='center', fig.cap='Productos más vendidos.'}
library(ggplot2)

# Ordenar por unidades vendidas y seleccionar los 5 productos más vendidos
top_productos <- df %>%
  arrange(desc(Cantidad)) %>%
  slice_head(n = 5)

# Asegurarse de que la columna Cantidad es numérica
Productos$Cantidad <- as.numeric(Productos$Cantidad)

# --- CÁLCULO DE LOS 5 PRODUCTOS MÁS VENDIDOS POR UNIDADES ---
...
# --- GRÁFICO DE BARRAS ---
ggplot(top_productos, aes(x = UnidadesVendidas, y = reorder(Producto, UnidadesVendidas), fill = Producto)) +
